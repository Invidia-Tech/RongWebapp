...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        hit_regex = re.compile(
            r'(?P<damage>[0-9]+)\s*\((?P<lap>[1-9][0-9]*)-(?P<boss>[1-9][0-9]*)(?:[,\/](?P<second_lap>[1-9][0-9]*)(?:-(?P<second_boss>[1-9][0-9]*))?)?\)')
            ...
            for day, col in enumerate(day_attack_columns):
                for attack in range(3):
                    ...
                    if matched:
                        ...
                        if hit_data['second_lap'] and (hit_data['second_lap'] < hit_data['lap'] or (
                                hit_data['lap'] == hit_data['second_lap'] and hit_data['boss'] > hit_data[
                            'second_boss'])):
        ...
        if not errors:
            ...
            with transaction.atomic():
                ...
                for day in range(5):
                    ...
                    waiting_hits = []
                    while current_hits:
                        normal_hits = [hit for hit in current_hits if (
                                    hit['lap'] == battle.current_lap and hit['boss'] == battle.current_boss and hit[
                                'second_lap'] is None)]
                        if normal_hits:
                            ...
                            killing_hit = [hit for hit in current_hits if
                                           (hit['lap'] == battle.current_lap and hit['boss'] == battle.current_boss)]
                            if not killing_hit:
                                raise ValueError("No killing hit found for lap %d, boss %d" % (
                                battle.current_lap, battle.current_boss))
                            if len(killing_hit) > 1:
                                raise ValueError("Multiple killing hits found for lap %d, boss %d" % (
                                battle.current_lap, battle.current_boss))
                            khit_data = killing_hit[0]
                            if khit_data['damage'] < battle.current_hp:
                                raise ValueError("Killing hit for lap %d boss %d not doing enough damage (%d < %d)" % (
                                battle.current_lap, battle.current_boss, khit_data['damage'], battle.current_hp))
                            kh_data = {'member': khit_data['member'], 'damage': battle.current_hp}
                            apply_hit(battle, kh_data, day)
                            waiting_hits.append(
                                {'member': khit_data['member'], 'damage': khit_data['damage'] - kh_data['damage'],
                                 'lap': khit_data['second_lap'], 'boss': khit_data['second_boss']})

                            wh_apply = [hit for hit in waiting_hits if
                                        (hit['lap'] == battle.current_lap and hit['boss'] == battle.current_boss)]
