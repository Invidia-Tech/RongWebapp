...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        battle = ClanBattle.objects.filter(id=battle.id).prefetch_related('flights', 'flights__pilot',
                                                                          'flights__pilot__user').first()
        stats = {}
        for flight in battle.flights.all():
            if flight.pilot_id not in stats:
                stats[flight.pilot_id] = {"name": flight.pilot.user.name, "landed": 0, "amb": 0, "canceled": 0,
                                          "crashed": 0, "in flight": 0, "total": 0, "total_complete": 0}
