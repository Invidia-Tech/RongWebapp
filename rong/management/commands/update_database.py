...
class Command(BaseCommand):
    ...
    def download(self, version):
        ...
        if version == 'en':
            ...
            while fails < 20:
                ...
                with requests.get('http://assets-priconne-redive-us.akamaized.net/dl/Resources/%d/Jpn/AssetBundles/iOS/manifest/manifest_assetmanifest' % test_version) as r:
                    if r.status_code == 200:
                        print("[SUCCESS] %d returned status code 200 (valid version)" % test_version)
                        truth_version = test_version
                        fails = 0
                    else:
                        fails += 1
                    test_version += 10
            ...
            with requests.get('http://assets-priconne-redive-us.akamaized.net/dl/Resources/%d/Jpn/AssetBundles/iOS/manifest/masterdata_assetmanifest' % truth_version) as rm:
                manifest = rm.text
                dbhash = manifest.split(",")[1]
                print('http://assets-priconne-redive-us.akamaized.net/dl/pool/AssetBundles/'+dbhash[0:2]+'/'+dbhash)
                        ...
                        for object in env.objects:
                            if object.type == 'TextAsset':
                                ...
                                with open("./redive_dbs/%s.db" % version, "wb") as fh3:
                                    ...
                                    print("Successfully downloaded db version %d" % truth_version)
                    else:
    ...
    def handle(self, *args, **options):
        ...
        with connection.cursor() as cur:
            ...
            cur.execute("SELECT EXISTS(SELECT FROM information_schema.schemata WHERE schema_name = %s)", [schema_name])
            ...
            if not os.path.exists('./redive_dbs/%s.db' % options['version']):
                raise CommandError("Database file not present for %s (try --download)" % options['version'])
...
WHERE constraint_type = 'FOREIGN KEY' AND ccu.table_schema = %s;
            ...
            for fkey in fkeys:
                cur.execute("ALTER TABLE {0}.{1} DROP CONSTRAINT {6}".format(*fkey))
            ...
            try:
                ...
                for fkey in fkeys:
                    cur.execute("ALTER TABLE ONLY {0}.{1} ADD CONSTRAINT {6} FOREIGN KEY({2}) REFERENCES {3}.{4}({5}) DEFERRABLE INITIALLY DEFERRED".format(*fkey))
